    ...
    if(playerManagedSessionBean.getUnigueIdentifer() == sharedApplicationSingleton.getUnigueIdentifier())
    {
        if (getGuess() == sharedApplicationSingleton.getNumberToGuess()) {
            if(!sharedApplicationSingleton.isDone)
            {
                ...
                // Add guess winner in DB
                Guesses guess = new Guesses();
                guess.setName(sharedApplicationSingleton.getName());
                guess.setGuesscount((short) sharedApplicationSingleton.getCount());
                guess.setGuessed((short) sharedApplicationSingleton.getNumberToGuess());
